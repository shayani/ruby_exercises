RSpec.describe 'inject pattern test' do
  ...
  it 'test 6' do
    ...
    toppings = []
    topping_calories.each do |topping, cal|
  ...
  it 'test 7' do
    ...
    sum_of_second_values = elements.flatten.select{|v| v.class == Integer }.inject(:+)
  ...
  it 'test 9' do
    ...
    avg = []
    grades.each do |k,v|
      avg.append((v.inject(:+).to_f / v.size.to_f).to_f)
    end

    final_grade = avg.inject(:+)
